...
class ModelFactory:
    ...
    def imagesNormalizer(image_input_path, image_output_path):
        ...
        for i in range(num_files):
            image = cv2.imread('{}/{}.jpg'.format(image_input_path, i)) #lê a imagem

            #Se a imagem não for carregada
            if image is None:
                print('Não foi possível ler a imagem: {}/{}.jpg'.format(image_input_path, i))
        ...
        for i in range(num_files):
            cv2.imwrite('{}/{}.jpg'.format(image_output_path, i), images[i] * 255)
